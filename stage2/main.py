...
def train(args, train_loader, model, criterion, criterion1, optimizer, epoch):
    ...
    for i, (input, target, target2) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        loss1.backward(
            retain_graph=True)  # you need to keep the graph from classification branch so that it can be used
...
def trainValidateSegmentation(args):
    ...
    # 클래스가 불균형하기 때문에 각 클래스에 가중치 부여 #default: 클래스 8개
    ...
    if args.onGPU == True:
        cudnn.benchmark = True  # 하드웨어에 맞게 자동 튜너 활성화
    ...
    # 학습률
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=args.step_loss, gamma=0.1)  # gamma=0.5
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        lossTr, overall_acc_tr, per_class_acc_tr, per_class_iu_tr, mIOU_tr, lossTr1, overall_acc_tr1, per_class_acc_tr1, per_class_iu_tr1, mIOU_tr1 = train(
            args, trainLoader, model, criteria, criteria1, optimizer, epoch)
        lossVal, overall_acc_val, per_class_acc_val, per_class_iu_val, mIOU_val, lossVal1, overall_acc_val1, per_class_acc_val1, per_class_iu_val1, mIOU_val1 = val(
            args, valLoader, model, criteria, criteria1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--scaleIn', type=int, default=1,
                        help='scaling factor for training the models at low resolution first and then full resolution. We did not use it.')
    ...
    parser.add_argument('--cached_data_file', default='ynet_joint_cache.p',
                        help='Data file names and other values, such as class weights, are cached')
